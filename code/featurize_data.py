...
import time
from featureFuncs import token_idx, read_glove, polarity_features, tense_features, distance_features
...
def create_features(ex, pos2idx, w2i, tokenizer, bert_model, ling_features):
    bert_model.eval()
    ...
    if ling_features:
        ...
        #new_fts.extend(polarity_features(ex['left_event'], ex['right_event']))

        #new_fts.extend(tense_features(ex['left_event'], ex['right_event']))

    return sent, bert_ent, bert_pos, new_fts, ex['rev'], lidx_start_s, lidx_end_s, ridx_start_s, ridx_end_s, pred_ind

def parallel(ex, ex_id, args, tokenizer, bert_model, ling_features):
    label_id = args._label_to_id[ex['rel_type']]
    return ex['doc_id'], ex_id, (ex['left_event'].id, ex['right_event'].id), label_id, \
        create_features(ex, args.pos2idx, args.w2i, tokenizer, bert_model, ling_features)
...
def main(args):
    ...
    with Pool() as pool:
        ...
        result = pool.starmap_async(parallel, [(v, k, args, tokenizer, bert_model,args.ling_features) for k, v in train_data.items()])
    ...
    if args.data_type in ['tbd']:
        ...
        dev_data = [parallel(v, k,args, tokenizer, bert_model,args.ling_features) for k,v in dev_data.items()]
        data += dev_data
    ...
    if 'all' in args.split:
        ...
        test_data = [parallel(v, k, args, tokenizer, bert_model, args.ling_features) for k,v in test_data.items()]
        print(len(test_data))
...
if __name__ == '__main__':
    ...
    p.add_argument('-ling_features', type=bool, default=True)
    args = p.parse_args()
