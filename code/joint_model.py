...
class EventDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        rels = sample['rels']
        # print(doc_id, context_id, context[0], context[1], context[2],  rels)
        # raise Exception
...
class ArcModule(nn.Module):
    def __init__(self, hidden_size, output_dim, dropout):
        ...
        # MLP classifier for head
        self.linear1_head = nn.Linear(self.hid_size, int(self.hid_size/2))
        self.act_head_1 = nn.ReLU()
        self.dropout_head_1 =  nn.Dropout(dropout)
        self.linear2_head = nn.Linear(int(self.hid_size/2), output_dim)
        self.act_head_2 = nn.ReLU()
        self.dropout_head_2 =  nn.Dropout(dropout)

        # MLP classifier for child dependent
        self.linear1_dep = nn.Linear(self.hid_size, int(self.hid_size/2))
        self.act_dep_1 = nn.ReLU()
        self.dropout_dep_1 =  nn.Dropout(dropout)
        self.linear2_dep = nn.Linear(int(self.hid_size/2), int(self.hid_size/4))
        self.act_dep_2 = nn.ReLU()
        self.dropout_dep_2 =  nn.Dropout(dropout)

        self.attention = Biaffine(int(self.hid_size/4),2)

    def forward(self, x):


        output_head = self.dropout_head_1(self.act_head_1(self.linear1_head(x)))
        output_head = self.dropout_head_2(self.act_head_2(self.linear2_head(output_head)))

        output_dep = self.dropout_dep_1(self.act_dep_1(self.linear1_dep(x)))
        output_dep = self.dropout_dep_2(self.act_dep_2(self.linear2_dep(output_dep)))




        output_attention = self.attention(output_head, output_dep)

        output_attention = torch.sum(output_attention, dim = -1)
        output_attention = output_attention.transpose(1,2)
...
class BertClassifier(nn.Module):
    ...
    def __init__(self, args):
        ...
        if args.biaffine:
            self.arc = ArcModule(self.hid_size * 2, int(self.hid_size / 2), 0.3)

        if args.multihead:
            self.multihead = nn.MultiheadAttention(self.hid_size * 2, num_heads=20, batch_first=True)
            self.layernorm = nn.LayerNorm(self.hid_size * 2)

    def forward(self, sents, lengths, fts = [], rel_idxs=[], lidx_start=[], lidx_end=[], ridx_start=[],
                ridx_end=[],  task='relation'):
        ...
        if task == 'entity':
            if args.biaffine and args.multihead:
                out_att, _ = self.multihead(out, out, out)
                out = out_att + out
                out = self.layernorm(out)

                out_ent = self.arc(out)
...
class NNClassifier(nn.Module):
    ...
    def construct_relations(self, ent_probs, lengths, rels, doc, poss, gold=False, train=True):
        ...
        for epoch in range(args.epochs):
            ...
            if len(eval_data) > 0:
                ...
                if args.data_type in ['red', 'caters']:
                    pred_labels = [pred_labels[k] if v == 1 else self._label_to_id['NONE'] for k,v in enumerate(self.pred_inds)]
...
if __name__ == '__main__':
    ...
    p.add_argument('-multihead',action='store_true', help='Use multihead attention on BiLSTM output', default=False)
    p.add_argument('-biaffine', action='store_true', help='Use biaffine attention', default=False)

    p.add_argument('-attention_type', type = str, )
    args = p.parse_args()
